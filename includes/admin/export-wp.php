 ...
 *
 * The function is borrowed and re-written from WP core.
 *
 ...
 * }
 */
function leco_cp_export_wp( $args = array() ) {
	global $wpdb, $post;

	$defaults = array( 'content' => 'leco-cp', 'author' => false, 'category' => false,
	...
	$args['leco_client'] = sanitize_text_field( $_GET['leco_client'] );
	if ( 'all' === $args['leco_client'] ) {
		$where = "{$wpdb->posts}.post_type IN ('leco_content_page', 'leco_client', 'leco_template')";

		// Grab a snapshot of post IDs, just in case it changes during the export.
		$post_ids = $wpdb->get_col( "SELECT ID FROM {$wpdb->posts} WHERE $where ORDER BY FIELD( post_type, 'leco_content_page', 'leco_template', 'leco_client' ) ASC" );
	} else {
		$_post_ids = leco_cp_get_projects_by_client( $args['leco_client'], array( 'leco_client', 'leco_template' ) );

		$where = "{$wpdb->posts}.post_type = 'leco_content_page'";
		$post_ids = $wpdb->get_col( "SELECT ID FROM {$wpdb->posts} WHERE $where" );

		$content_page_ids = array();
		foreach ( $post_ids as $content_page ) {
			$projects = leco_cp_get_projects_by_content_page( $content_page );
			foreach ( wp_list_pluck( $projects, 'ID' ) as $project ) {
			    if ( in_array( intval( $project ), $_post_ids, true ) ) {
				    $content_page_ids[] = $content_page;
                }
            }
        }

		$post_ids = array_merge( $content_page_ids, $_post_ids );
	}

	$where          = "WHERE {$wpdb->posts}.post_type = 'attachment' AND {$wpdb->posts}.post_parent IN (" . join( ',', $post_ids ) . ")";
	$attachment_ids = $wpdb->get_col( "SELECT ID FROM {$wpdb->posts} $where" );
		...
		<channel>
			...
			<?php if ( $post_ids ) {
				...
				while ( $next_posts = array_splice( $post_ids, 0, 20 ) ) {
					...
					$posts = $wpdb->get_results( "SELECT * FROM {$wpdb->posts} $where ORDER BY FIELD( post_type, 'attachment', 'leco_content_page', 'leco_template', 'leco_client' ) ASC" );
					...
					foreach ( $posts as $post ) {
						...
						<item>
							...
							if ( apply_filters( 'leco_cp_can_export_comments', false ) ) {
